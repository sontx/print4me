  ...
  [ALGORITHM_BINARY_TREE]: {
    ...
    fn: function* (grid, config) {
      ...
      for (let i = 0; i < allCoords.length; i++) {
        const cell = grid.getCellByCoordinates(allCoords[i]),
        ...
        for (let x = 0; x < grid.metadata.width; x++) {
          ...
          if (goEast) {
            ...
            const randomCellFromRun = random.choice(currentRun),
              southNeighbourOfRandomCell =
                randomCellFromRun.neighbours[DIRECTION_SOUTH];
    ...
    fn: function* (grid, config) {
      ...
      while (true) {
        ...
        if (nextCell) {
          ...
          const unvisitedCellWithVisitedNeighbours = grid.randomCell(
            (cell) => isUnvisited(cell) && cell.neighbours.random(isVisited),
          );
          if (unvisitedCellWithVisitedNeighbours) {
            const visitedNeighbour =
              unvisitedCellWithVisitedNeighbours.neighbours.random(isVisited);
    ...
    fn: function* (grid, config) {
      ...
      while (active.length) {
        const randomActiveCell = random.choice(active),
          randomInactiveNeighbour =
            randomActiveCell.neighbours.random(isUnvisited);
  ...
  [ALGORITHM_TRUE_PRIMS]: {
    ...
    fn: function* (grid, config) {
      ...
      function getCellWithLowestCost(cells) {
        return cells.sort(
          (n1, n2) => n1.metadata[METADATA_COST] - n2.metadata[METADATA_COST],
        )[0];
      }
      ...
      while (active.length) {
        const randomActiveCell = getCellWithLowestCost(active),
          inactiveNeighbours = randomActiveCell.neighbours
            .toArray()
            .filter(isUnvisited);
        if (!inactiveNeighbours.length) {
          ...
          const inactiveNeighbourWithLowestCost =
            getCellWithLowestCost(inactiveNeighbours);
      ...
      function mergeCellsInRow(y, oddsOfMerge = 1) {
        for (let i = 0; i < width - 1; i++) {
          ...
          if (cell1SetId !== cell2SetId && random.int(oddsOfMerge) === 0) {
            grid.link(cell1, cell2);
            mergeSets(
              cell1.metadata[METADATA_SET_ID],
              cell2.metadata[METADATA_SET_ID],
            );
          }
        }
      }
